from tkinter import*
root = Tk()
Greeting = Label(root, text = "Hello User, please press Next and Type in your Name").grid(row= 0, column= 0)
NameEntry = Entry(root)
def ShowNameInput():
    NameEntry.grid(row=2, column=0)
    FirstSaveClick = Button(root, text = "Save", command = SaveUsername).grid(row=3, column= 0)
    ShowListClick = Button(root, text = "Show List of Usernames", command = printUsernames).\
        grid(row=4, column = 0)
